...
//4:遍历ASCII的utf8
var strUnicodeScalars = Array(str.utf8)
for charNum in strUnicodeScalars
{
    print("\(charNum) ", terminator: "")
}
//5:使用下标索引
for num in 0..<str.count
{
    let char:Character = str[str.index(str.startIndex,offsetBy: num)]
    print("\(char) ", terminator: "")
}

//方式2:建分类索引
extension String {
    //subscript函数能够检索数组中的值
    //按照索引方式读取字符
